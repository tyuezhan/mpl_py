...
def Astar(start_coord, ENV, ss, traj, max_expand=-1, verbose=False):
    ...
    while ss.pq:
        ...
        if ENV.plan_timeout():
            ...
            recover_traj(best_node, ss, ENV, start_coord, traj)
            return best_node.g
    ...
    if recover_traj(curr_node, ss, ENV, start_coord, traj):
        return curr_node.g
    else:
        return float('inf')
    

def recover_traj(curr_node, ss, ENV, start_key, traj, verbose=False):
    print("--------------------------------------------")
